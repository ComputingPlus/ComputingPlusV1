...
if ($stmt->num_rows > 0) {
    // Update the record to unlocked.
    $stmt->close();
    $stmt = $conn->prepare("UPDATE homework_lock_status SET is_locked = 0, locked_at = NULL WHERE homework_id = ? AND student_id = ?");
    $stmt->bind_param("ii", $homework_id, $student_id);
    $stmt->execute();
    $stmt->close();
} else {
    $stmt->close();
    // Optionally, insert a new record as unlocked.
    $stmt = $conn->prepare("INSERT INTO homework_lock_status (homework_id, student_id, is_locked) VALUES (?, ?, 0)");
    $stmt->bind_param("ii", $homework_id, $student_id);
    $stmt->execute();
    $stmt->close();
}

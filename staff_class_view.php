...
// Process set class assessment
if (isset($_POST['set_class_assessment'])) {
    $assessmentName = $_POST['assessment_name'];
    $stmt = $conn->prepare("INSERT INTO class_assessments (class_id, assessment_name, created_at) VALUES (?, ?, NOW())");
    $stmt->bind_param("is", $class_id, $assessmentName);
    $stmt->execute();
    $stmt->close();
}

// Retrieve student list for this class
$classStudents = [];
...
// Retrieve class assessments for this class
$stmt = $conn->prepare("SELECT * FROM class_assessments WHERE class_id = ? ORDER BY created_at DESC");
$stmt->bind_param("i", $class_id);
$stmt->execute();
...
<div class="content">
    ...
    <button class="button" onclick="toggleSidebar('classAssessmentSidebar')">Set Class Assessment</button>
    ...
    <table class="table">
        ...
        <tr>
            ...
            <td>
                <a href="staff_homework_submission.php?homework_id=<?php echo $hw['id']; ?>">
    ...
    <table class="table">
        ...
        if ($assessmentResult->num_rows > 0) {
            while ($assess = $assessmentResult->fetch_assoc()) { ?>
            <tr>
                <td><?php echo htmlspecialchars($assess['assessment_name']); ?></td>
                <td>
                    <!-- Link to view assessment page. Replace "Enter.ico" with your icon as needed -->
                    <a href="view_assessment.php?assessment_id=<?php echo $assess['id']; ?>">
                        <button class="button">
                            <img src="Enter.ico" alt="Enter" style="height:16px;vertical-align:middle;"> Enter
                        </button>
                    </a>
                </td>
            </tr>
        <?php }
            ...
            <tr><td colspan="2">No assessments added yet.</td></tr>
        <?php } ?>
    </table>
</div>
...
<!-- Sidebar for setting class assessment -->
<div class="sidebar" id="classAssessmentSidebar">
    <h3>Set Class Assessment</h3>
    <form method="post">
        <label>Class Name</label>
        <input type="text" value="<?php echo htmlspecialchars($class['class_name']); ?>" disabled>
        <!-- Pass along the class_id implicitly (the page already knows it) -->
        <label>Assessment Name</label>
        <input type="text" name="assessment_name" required>
        <button type="submit" name="set_class_assessment" class="button">Submit</button>
        <button type="button" class="button" onclick="toggleSidebar('classAssessmentSidebar')">Close</button>
    </form>
</div>
